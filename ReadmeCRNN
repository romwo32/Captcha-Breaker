...
After installing the required packages, we can download the dataset we'll use to train our model. Dataset you can download from this https://github.com/AakashKumarNain/CaptchaCracker/raw/master/captcha_images_v2.zip link. The dataset contains 1040 captcha files as png images. The label for each sample is a string, the file's name (minus the file extension).
...
After running this script, the required dataset will be saved into a folder called "Datasets". If we looked at it, we would see the following images: alt text
...
train_workers is an integer that may represent the number of worker threads or processes used to parallelize the training process. Now we need to create our data provider, where we'll do complete data preprocessing while training and validating our model:
...
Now we have a split dataset where 90% is for training and 10% for validation. But you may already notice that I was not using any augmentation techniques because test data shouldn't be augmented when validating. So, we need to add augmentors only to the training data provider:
...
You may notice that I am tracking "val_CER" instead of loss, so let's check how my training and validation CER curves look like: alt image
...
It loads the model configuration file and creates an instance of the ImageToWordModel object using the model_path and char_list arguments from the configuration file. The ImageToWordModel object is a subclass of the OnnxInferenceModel class, which loads an ONNX model and provides a prediction method for making predictions with the model. The ImageToWordModel class adds a char_list attribute and overrides the predict method to preprocess the input image and decode the model's output;

It loads the validation data from a CSV file and converts it to a list of tuples. The validation data consists of image file paths and corresponding text labels;

It iterates through the validation data and uses the model.predict method to get the predicted text for each image. The function computes the character error rate (CER) between the predicted text and the ground truth label. The CER measures the difference between the predicted text and the label, expressed as a percentage;

It prints the image file path, label, prediction, and CER for each image, and at the end of the loop, it computes and prints the average CER for the entire dataset.
...
Let's see what the results of this model are within our validation dataset: alt image
